function probs = est_p(X, N_s, tau)

%-------------------------------------------------------------------------------------------------
% From phi_toolbox - modified to compute distributions across several
% trials
%
...
%
% INPUTS:
%   X: discretized time series data in form units x time x trials
...
for trial = 1 : Trials
    for t=1: T-tau
        past_s = baseMto10_table(X(:,t,trial) - 1, pow_vec) + 1; % past state
        p_past(past_s) = p_past(past_s) + 1;
        present_s = baseMto10_table(X(:,t+tau,trial) - 1, pow_vec) + 1; % present state
...
p_past = p_past/trans_counter;
p_present = p_present/trans_counter;
p_joint = p_joint/trans_counter;
