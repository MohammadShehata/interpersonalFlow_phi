...
out_dir = '4ch_diff_perSong_2t2_2chOrder/';
...
%% Sort pairs, use every combination of pairings
...
%% Get combinations of top 2-channel pairs

ranks = 10;

sets = nchoosek((1:ranks), 2);
part_networks = cell(1, 2); % networks x participants

set_counter = 1;
for net_c = 1 : size(sets, 1)
    net = net_map_sorted(sets(net_c, :), :);
    
    % Ignore set if a channel is repeated for a participant
    if ~any(diff(net, [], 1) == 0)
        part_networks{set_counter, 1} = net(:, 1);
        part_networks{set_counter, 2} = net(:, 2);
