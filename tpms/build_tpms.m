...
Builds a single TPM for each condition, across all trials
...
for pair = 1 : nPairs
    for condition = 1 : nConditions
        ...
        % Join trials together
        for part = 1 : 2
            channels_per_participant(part) = size(pair_data{part}{1}, 1); % assumes constant number of channels across trials
            pair_data{part} = cat(3, pair_data{part}{:});
        end
        
        % Join participants together
        pair_data = cat(1, pair_data{:});
        
        % All possible combinations of channels
        networks = nchoosek((1:size(pair_data, 1)), nChannels);
        
        pair_data = permute(pair_data, [2 1 3]); % (time x channels x trials)
        ...
        parfor multiplier = 1 : length(tau_multipliers) % ~90 seconds per tau for pair 1
            ...
            % Downsample
            pair_data_down = downsample_meanShift(pair_data, tau_multiplier);
            ...
            out_file = [cond_names{condition} '_p' num2str(pair) 'tau' num2str(tau) '.mat'];
