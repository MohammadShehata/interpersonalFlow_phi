...
for fixed_channel = 1 : length(fixed_channels)
    ...
    a = repmat(valid_channels, [length(valid_channels) 1]);
    b = repmat(valid_channels, [1 length(valid_channels)]);
    channel_selections = [a(:) b(:)];
    
    % Remove duplicates
    channel_selections = sort(channel_selections, 2);
    channel_selections = unique(channel_selections, 'rows');
    
    % Combine with fixed channels
    for c = 1 : size(channel_selections, 1)
        
        if all(diff(channel_selections(c, :)) == 0)
            
            % No need to swap participants
            part_networks{net_c, 1} = sort([fixed_channels(fixed_channel) channel_selections(c, 1)]);
            part_networks{net_c, 2} = sort([fixed_channels(fixed_channel) channel_selections(c, 2)]);
            net_c = net_c + 1;
            ...
            % 1-3
            part_networks{net_c, 1} = sort([fixed_channels(fixed_channel) channel_selections(c, 2)]);
            part_networks{net_c, 2} = sort([fixed_channels(fixed_channel) channel_selections(c, 1)]);
            net_c = net_c + 1;
        end
